...
function renderLicenses() {
  ...
  if (!licenses.length) {
    elements.licensesBody.innerHTML = `
      <tr>
        <td colspan="6" class="empty-cell">No licenses match your current filters.</td>
      </tr>
    `;
    return;
  }
...
async function loadDashboard() {
  ...
  try {
    ...
    if (!statsResponse.ok || !licensesResponse.ok) {
      throw new Error("Failed to load dashboard data.");
    }

    state.stats = await statsResponse.json();
    const licensePayload = await licensesResponse.json();
    ...
    elements.licensesBody.innerHTML = `
      <tr>
        <td colspan="6" class="empty-cell">Unable to load dashboard data.</td>
      </tr>
    `;
    elements.productsGrid.innerHTML = `
  ...
  try {
    ...
    setValidationResult(
      `
        ...
        <p>${escapeHtml(payload.status)} • ${escapeHtml(payload.product)}</p>
...
async function revokeLicense(licenseId) {
  ...
  try {
    const response = await fetch("/licenses/revoke", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      throw new Error(payload?.error || "Failed to revoke license.");
    }
